...
//记录用户反馈key值（仅第一次播放时记录）
var feedKey=true;
//记录用户是否初次播放
var onekey=true;
//获取设备类型（a:安卓；i:ios;h:其他）
...
function createPlayerLisSection(videoUrl,videoThumb,videoName,videoPraiseNum){
        ...
        events: {
            ...
            onPlay:function(){
                if(phoneType=="a"){
                    ...
                    $(".test_header_top_text").text(toStrNum(videoName,70));
    ...
    jwplayer().onPlay(function(){
        ...
        //控制控制栏显示隐藏
        var time=setTimeout(hidden,5000);
        function hidden(){
            $(".jw-controlbar").removeAttr("style");
            clearTimeout(time);
        };
        //记录视频播放log
        timeoutId=setInterval(recordLogs,duration*1000);
